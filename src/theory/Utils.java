...
public class Utils {
    ...
    public static <T> int binSearch(List<T> array, T element, Comparator<T> comparator, int lo, int hi) {
        if (lo < hi) {
            int mid = (lo / 2) + (hi / 2);
            int cmp = comparator.compare(array.get(mid), element);
            if (cmp < 0) return binSearch(array, element, comparator, mid + 1, hi);
            if (cmp > 0) return binSearch(array, element, comparator, lo, mid - 1);
            return mid;
        }
        if (array.get(lo).equals(element)) {
            return lo;
        }
        return -1;
    }
